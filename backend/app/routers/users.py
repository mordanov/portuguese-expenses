...
async def create_new_user(
    ...
    user = await create_user(session, body.username, hash_password(body.password), body.role)
...
async def update_existing_user(
    ...
    user = await update_user(session, user, body.username, password_hash, body.role, body.is_active)
    await session.commit()
    return UserResponse.model_validate(user)
